...
### 법령 검색 — `ConnectionResetError(104, 'Connection reset by peer')`

**원인**: 국가법령정보센터 API는 키 발급 후 **사용 도메인을 별도 등록**해야 합니다.  
도메인 미등록 상태에서 서버 외부로 요청하면 연결이 강제 차단됩니다.

**도메인 등록이 필요한 경우**

| 환경 | 등록 필요 여부 | 비고 |
| ---- | -------------- | ---- |
| Streamlit Cloud (공개 배포) | ✅ 필요 | 배포 도메인(예: `hytc-dart.streamlit.app`) 등록 |
| 사내 서버 / 외부 호스팅 | ✅ 필요 | 실제 서비스 도메인 등록 |
| 로컬 개발 (`localhost`) | ❌ 불필요 | OC 키만으로 동작 |

**등록 방법**: [법령 API 포털](https://www.law.go.kr/LSW/openApi.do) → OPEN API 신청 → 도메인 등록 → 승인 확인 후 사용

## 📋 패치 내역

### v0.5.0
- **feat**: 재무제표 수집 진행률 표시 — 프로그레스바 + `수집 중... (N/M) 연도 - 기업명` 상태 텍스트
- **feat**: 수집 완료 후 데이터 미리보기 — 다운로드 버튼 위 전체 행 수 + DataFrame 표시
- **fix**: 수집 메시지에 종목코드 대신 기업명 표시 (삼성SDI, LG에너지솔루션 등)
