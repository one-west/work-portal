...
API 키는 `.env`에 설정 (`.env`는 gitignore됨):
```
DART_API_KEY=...
LAW_API_KEY=...
```
...
수집 흐름: API 키 로드 → corp_codes 캐시 복사 → `init_dart()` → UI → 버튼 클릭 → `finstate_all()` → XBRL 없으면 `finstate()` fallback (CFS→OFS) → 엑셀 저장

## OpenDartReader 캐시

`docs_cache/opendartreader_corp_codes_{YYYYMMDD}.pkl` 형식으로 일별 캐시. Streamlit Cloud 파일시스템은 재시작마다 초기화되므로, 앱 시작 시 `docs_cache/`의 기존 pkl을 오늘 날짜로 복사해 재사용. 캐시가 오래되면 최신 pkl로 교체 후 커밋.
