...
주요 함수:
- `to_number_strict(x)` — DART API 반환 숫자 문자열 정규화. 유니코드 공백 14종, 삼각형(△▲) 음수, 괄호 `(n)` 음수, en/em dash 처리
...
수집 흐름: API 키 로드 → corp_codes 캐시 복사 → `init_dart()` → UI → 버튼 클릭 → 프로그레스바 표시 → `finstate_all()` → XBRL 없으면 `finstate()` fallback (CFS→OFS) → 숫자 정규화 → 엑셀 저장 → DataFrame 미리보기 → 다운로드 버튼
...
- `finstate_all()` — XBRL 기반 API(`fnlttXbrlDpSj`). 당일 제출 보고서 또는 일부 분기 보고서는 XBRL 처리 완료 전 빈 결과 반환. **API 레벨에서 가장 완전한 방법**이지만 DART 인덱싱 누락 항목(예: `ifrs-full_ProfitLoss`)은 반환되지 않을 수 있음
- `finstate()` — non-XBRL fallback(`fnlttSinglAcntAll`). 주요 계정 항목만 반환 (finstate_all보다 적음). `finstate_all()` 빈 결과 시 CFS → OFS 순으로 시도
- 완전한 XBRL 전 항목이 필요하면 `finstate_xml()`로 원본 ZIP 파싱 필요 (현재 미구현)
